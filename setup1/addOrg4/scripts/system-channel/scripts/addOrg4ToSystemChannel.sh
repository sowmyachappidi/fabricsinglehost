...
export CHANNEL_NAME=org1-org4-channel
...
fetchChannelConfig() {
    ...
    jq -s '.[0] * {"channel_group":{"groups":{"Consortiums":{"groups":{"SampleConsortium":{"groups": {"Org4MSP":.[1]}}}}}}}' config.json ./org4.json >modified_config.json
}
...
signConfigAsOrdererOrg() {
    ...
    setGlobalsForOrderer
    peer channel update -f org4_update_in_envelope.pb -c ${SYSTEM_CHANNEL_NAME} \
...
updateAnchorPeers(){
    ...
    peer channel update -o localhost:7050 \
    --ordererTLSHostnameOverride orderer.example.com \
    -c $CHANNEL_NAME -f ./Org4MSPanchors.tx \
    --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA
